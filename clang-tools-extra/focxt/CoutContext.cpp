...
// bool SignatureApplication::has(std::string file_path, std::string class_name,
...
void SignatureApplication::append(std::string file_path, std::string class_name,
                                  std::string signature) {
  bool b = 0;
  for (auto &file : files) {
    if (file.file_path == file_path) {
...
json SignatureContext::get_j() {
  ...
  j[file_path][class_name + "::" + function_name][signature] = json::object();
  ...
  for (int i = 0; i < classes.size(); i++) {
    ...
    if (j[file_path][class_name + "::" + function_name][signature]
         [classes[i].its_namespace]
             .find("class") == j[file_path][class_name + "::" + function_name]
...
SignatureContext get_signature_context(std::vector<FileContext> &file_contexts,
  ...
  for (auto file_context : file_contexts) {
    if (file_context.file_path == file_path) {
      ...
      get_all_applications(applications, file_contexts, file_path, class_name,
                           signature);
        ...
        for (auto a_class : file.classes) {
          if (a_class.class_name == "class") {
            ...
            for (auto signature : a_class.signatures) {
              ...
              } else if (file_context.class_has_destructor(a_class.class_name,
                                                           signature)) {
                signature_context.classes[i].destructor =
                    file_context.class_get_destructor(a_class.class_name,
                                                      signature);
                                                       ...
                                                       signature)) {
                for (int j = 0; j < signature_context.classes[i].methods.size();
                     j++) {
